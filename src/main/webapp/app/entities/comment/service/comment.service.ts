...
type RestOf<T extends IComment | NewComment> = Omit<T, 'createdDate'> & {
...
export class CommentService {
  ...
  query(req?: { page: number; size: number; eagerload: boolean; sort: string[] }): Observable<EntityArrayResponseType> {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      createdDate: comment.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      createdDate: restComment.createdDate ? dayjs(restComment.createdDate) : undefined,
    };
  }
