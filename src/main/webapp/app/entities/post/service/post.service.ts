...
type RestOf<T extends IPost | NewPost> = Omit<T, 'createdDate'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      createdDate: post.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      createdDate: restPost.createdDate ? dayjs(restPost.createdDate) : undefined,
    };
  }
